    ...
    if request.method == "POST":
        ...
        if request.form['submittionType'] == 'createClass':
            ...
            db = sqlite3.connect(os.path.dirname(
                os.path.abspath(__file__)) + '/data/database.db')
            dbcmd = db.cursor()
            prevClassName = dbcmd.execute(
                "SELECT * FROM classes WHERE name=?", [className]).fetchone()
            db.close()
            if prevClassName:
                sD.studentDict[request.remote_addr]['class'] = className
                if forward:
        ...
        if request.method == 'POST':
            ...
            if userType == "login":
                ...
                if userFound:
                    ...
                    userFound = dbcmd.execute("INSERT INTO users (username, password, permissions, bot) VALUES (?, ?, ?, ?)", (
                        username, passwordCrypt, sD.settings['perms']['anyone'], str(bot)))
                    db.commit()
                    db.close()
                    newStudent(remote, username, bot=bot)
                    if forward:
